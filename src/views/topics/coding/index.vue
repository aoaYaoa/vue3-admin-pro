...
import { topicList, type Topic } from './data'
...
const functionalTopics = computed(() => {
  if (!searchText.value) {
    return topicList.filter((topic: Topic) => !topic.category || topic.category !== '算法题')
  }
  const keyword = searchText.value.toLowerCase()
  return topicList.filter(
    (topic: Topic) => 
...
const algorithmTopics = computed(() => {
  if (!searchText.value) {
    return topicList.filter((topic: Topic) => topic.category === '算法题')
  }
